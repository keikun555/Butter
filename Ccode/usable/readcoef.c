...
int strdecomp(char *string1, char *string2)
{
  int i, j, k, l, m, n;
  float aa, bb, cc, dd, ee, ff, gg, xx, yy, zz;

  i = 0; n = 0;
  while (*(string1+i) != '\0')
    if (*(string1+(i++)) == '\n') n = 1;
  if (n == 0) return -2; /* No RETURN */

  i = 0;
  j = 0;
  while ((*(string1+i) == ' ') || (*(string1+i) == '\t')) i++;
  if (*(string1+i) == '\n') return -1; /* No items */
  while ((*(string1+i) != ' ') && (*(string1+i) != '\n') && (*(string1+i) != '\t'))
    *(string2+(j++)) = *(string1+(i++));
  *(string2+j) = '\0';
  while (((*(string1+i) == ' ') || (*(string1+i) == '\t')) && (*(string1+i) != '\0')) i++;
  k = 0;
  while (*(string1+i) != '\0') *(string1+(k++)) = *(string1+(i++));
  *(string1+k) = *(string1+i);
  return 1;
} /* int strdecomp(char *string1, char *string2) */
...
char *filtername;
{
 ...
 coefline = 0; /* lowpass00 = 0; highpass00 = 0; notch00 = 0; bandstop00 = 0; */
 while (1)
 {
  do
  {
